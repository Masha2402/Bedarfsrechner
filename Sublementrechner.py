...
def berechne_werte():
    ...
    if geschlecht == "Männer":
        eaa = round(gewicht * 0.2, 2)
        bcaa = round(gewicht * 15, 2)
        ...
        eaa = round(gewicht * 0.1, 2)
        bcaa = round(gewicht * 5, 2)
        protein = round(gewicht * 1.2, 2)
        kreatin = 3.5  # Kreatin nicht gerundet, da es eine feste Menge ist
        fett_prozent = 20
        kohlenhydrate_prozent = 30
        wasser = 2.2  # Wasser nicht gerundet, da es eine feste Menge ist

    fett = round((fett_prozent / 100) * (eaa + bcaa + protein), 2)
    kohlenhydrate = round((kohlenhydrate_prozent / 100) * (eaa + bcaa + protein), 2)

    ergebnis_text.config(state=tk.NORMAL)
    ergebnis_text.delete(1.0, tk.END)
    ergebnis_text.insert(tk.END, f"EAA: {eaa}g\n")
    ergebnis_text.insert(tk.END, f"BCAA: {bcaa}g\n")
